...
if file.endswith(".csv"):
    ...
    if os.stat(fileName).st_size != 0:
        ...
        plt.plot(utc_time, data_stream, label="BCG Signal", color="blue", linewidth=1)
        plt.xlabel("Time (s)")
        plt.ylabel("Amplitude")
        plt.title("BCG Signal vs Time")
        plt.legend()
        plt.grid()
        plt.savefig("bcg_signal_plot2.png", dpi=300)  # Save the plot as an image
